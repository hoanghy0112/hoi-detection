    ...
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={styles.mainBox}>
          {predictionStatus === 'succeeded' ? (
            predictionResults?.map?.(({label, verb, score}, index) => (
              <TouchableOpacity
                key={`${label}-${verb}-${score}`}
                onPress={() => {
                  setVerbIndex(index);
                  setScrollOffsetY(initialContentOffset);
                  manBoxScrollRef.current.scrollTo({
                    x: 0,
                    y: initialContentOffset,
                    animated: true,
                ...
                <View style={styles.verbButton}>
                  <Text style={styles.verb}>{`${verb
                    .split('_')
                    .join(' ')} - ${label}`}</Text>
                  <Text style={styles.score}>
                    {Number.parseFloat(score).toFixed(4)}
                  </Text>
                </View>
              </TouchableOpacity>
            ))
          ...
          <View style={styles.buttonGroup}>
            <RoundedButton
              ...
              icon={<SVG_DOWNLOAD width={40} height={40} />}
              text="Download"
            />
            <RoundedButton
              style={styles.statisticButton}
              icon={<SVG_CHART width={40} height={40} />}
              text="Statistic"
            />
            <RoundedButton
